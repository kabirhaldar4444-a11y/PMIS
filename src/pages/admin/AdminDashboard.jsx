      ...
      <nav className="fixed top-6 left-1/2 -translate-x-1/2 w-[95%] max-w-6xl z-50 pointer-events-none">
        <div className="bg-white/70 backdrop-blur-2xl rounded-full px-4 sm:px-8 py-2.5 flex items-center justify-between shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-white/50 ring-1 ring-slate-200/20 pointer-events-auto">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            <button 
              onClick={() => setActiveTab('service_delivery')} 
              className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-xs sm:text-sm font-black transition-all duration-300 ${activeTab === 'service_delivery' ? 'bg-gradient-to-r from-indigo-600 to-blue-600 text-white shadow-xl shadow-indigo-600/20' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-900'}`}
            >
              <Truck className={`w-4 h-4 ${activeTab === 'service_delivery' ? 'text-white' : ''}`} />
